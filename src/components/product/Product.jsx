...
const Product = () => {
  ...
  const { product, category, productEdit } = productState
  useEffect(() => {
  ...
  const handlerEnable = (product) => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire('Habilitado!', 'Producto habilitado con exito.', 'success')
      }
    })
  }
  return (
          ...
          return (
            <ContainerProduct key={category.id} id={category.name}>
              ...
              <ContainerProductSection>
                {product.length > 0 ? (
                  product.filter((product) => product.category === category.id)
                    .length > 0 ? (
                    product.map((e) => {
                ...
                {userState !== null ? (
                  userState.filter((rol) => rol.name === 'ROLE_ADMIN')
                    .length === 1 ? (
                    <ContainerCard>
                      <CardAdd>
                        <div>
                ...
                >
                  {category ? (
                    category.map((e, index) => {
                      return (
                        <option
                          selected={e.id === productEdit.category}
                          key={index}
                        >
                          {e.name}
                        </option>
